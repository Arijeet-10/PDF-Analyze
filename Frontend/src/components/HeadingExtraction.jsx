...
// A professional, modern UI for the PDF Document Analyzer on a single page
const HeadingExtraction = () => {
  ...
  const handleRemoveFile = (fileNameToRemove) => {
    // Revoke URL and remove from state
    ...
    // Remove the result associated with the file
    setResults((prevResults) =>
      prevResults.filter((r) => r.filename !== fileNameToRemove)
    );
  ...
  const handleHeadingClick = async (doc, heading) => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const resultOnPage = searchResults.find(
          (r) => r.page_num === heading.page + 1
        );
        ...
        if (resultOnPage && resultOnPage.quads.length > 0) {
          ...
          apis.gotoLocation(
            heading.page + 1,
            resultOnPage.quads[0][0],
            resultOnpage.quads[0][1]
          );
        } else {
          console.warn(
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://arijeey-10-pdf-analyze-backend.hf.space/api/pdf-outline",
        {
          method: "POST",
          body: formData,
        }
      ...
      setResults((prevResults) => {
        ...
        (newData || []).forEach((newResult) => {
          resultsMap.set(newResult.filename, newResult);
        });
      ...
      setExpandedDocs((prev) => ({ ...prev, ...initialExpandedState }));
    } catch (error) {
  ...
  const getHeadingTextStyle = (levelStr) => {
    ...
    switch (level) {
      case 1:
        return "text-slate-800 font-semibold text-sm";
      case 2:
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
        ...
        <div className="w-full max-w-4xl mx-auto">
          <header className="text-center mb-10">
            ...
            <p className="mt-3 text-lg text-slate-600 max-w-xl mx-auto">
              Instantly generate a clickable table of contents for any PDF
              document.
            </p>
          </header>
          ...
          <div className="bg-white rounded-2xl shadow-lg shadow-slate-200/60 border border-slate-200">
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-1 ml-4 flex-shrink-0">
                        <button
                          ...
                          className="p-1.5 text-slate-500 hover:text-indigo-600 transition-colors rounded-md hover:bg-slate-200"
                          title="Preview PDF"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 text-slate-500 hover:text-red-600 transition-colors rounded-md hover:bg-slate-200"
                          title="Remove File"
                        >
                          <X className="w-4 h-4" />
                        </button>
            ...
            <div className="p-6 bg-slate-50/70 border-t border-slate-200 rounded-b-2xl">
              {message && !isLoading && (
                <div
                  className={`mb-4 p-3 rounded-lg border flex items-start space-x-3 text-sm ${
      ...
      {results.length > 0 && (
        <main className="flex-1 flex flex-col lg:flex-row gap-4 p-4 min-h-0">
          ...
          <aside className="w-full lg:w-1/2 flex-shrink-0 flex flex-col bg-gradient-to-br from-white via-slate-50/30 to-slate-100/20 rounded-2xl shadow-xl shadow-slate-900/5 border border-slate-200/60 backdrop-blur-sm overflow-hidden">
            ...
            <div className="overflow-y-auto space-y-3 p-4">
              {results.map((doc, idx) => (
                <div
                  key={idx}
                  className="group bg-white/60 backdrop-blur-sm rounded-xl border border-slate-200/50 shadow-sm hover:shadow-md hover:shadow-slate-200/50 transition-all duration-300 hover:border-slate-300/60"
                >
                  <button
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-5 h-5 text-slate-400 transition-all duration-300 group-hover:text-indigo-600 ${
                        expandedDocs[doc.filename] ? "rotate-180" : ""
                      }`}
                    />
                  </button>
                      ...
                      {doc.outline?.outline?.length > 0 ? (
                        <div className="space-y-1 border-t border-slate-200/60 pt-4">
                          {doc.outline.outline.map((heading, hIdx) => (
                            <button
                              ...
                              className={`w-full flex items-center justify-between text-left py-2.5 px-3 rounded-lg hover:bg-indigo-50/80 hover:border-indigo-200/50 border border-transparent transition-all duration-200 group/heading ${getIndentLevelClass(
                                heading.level
                              )}`}
                            >
                              <span
                                className={`flex-1 truncate group-hover/heading:text-indigo-700 transition-colors ${getHeadingTextStyle(
                                  heading.level
                                )}`}
                              >
                                {heading.text}
                        ...
                        <div className="text-center py-8 px-4 border-t border-slate-200/60">
                          ...
                          <p className="text-slate-400 text-xs mt-1">
                            This document doesn't contain any structured
                            headings
                          </p>
                        </div>
                      )}
          ...
          <section className="w-full lg:w-1/2 h-[calc(100vh-64px)] flex bg-white rounded-xl shadow-lg shadow-slate-200/60 border border-slate-200 flex-col overflow-hidden">
            {selectedPdf ? (
              <>
